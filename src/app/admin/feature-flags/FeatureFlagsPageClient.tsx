  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>{initial ? "Edit Feature Flag" : "New Feature Flag"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <div>
            ...
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="My Feature" className="mt-1 h-8 text-sm" />
          </div>
          <div>
        ...
        <DialogFooter className="mt-2">
          ...
          <Button
            ...
            onClick={() => upsert.mutate({ key, name, rolloutPct: Number(pct) })}
          >
            {initial ? "Save" : "Create"}
          </Button>
        </DialogFooter>
...
export default function FeatureFlagsPageClient() {
  ...
  const { data, isLoading } = useQuery(trpc.admin.featureFlags.list.queryOptions());

  const toggleMutation = useMutation(
    trpc.admin.featureFlags.toggle.mutationOptions({
      onSuccess: () => qc.invalidateQueries(trpc.admin.featureFlags.list.pathFilter()),
      onError: (e) => toast.error(e.message),
    }),
  );
  ...
  return (
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {(data ?? []).map((flag) => (
            <Card key={flag.id} className={cn("border-border/50", !flag.enabled && "opacity-60")}>
              <CardContent className="flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                  <Switch
                    checked={flag.enabled}
                    onCheckedChange={() => toggleMutation.mutate({ key: flag.key })}
                  />
                  <div>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem className="text-red-600" onClick={() => setDeleteTarget(flag)}>
      ...
      {editTarget && (
        <FFDialog open={!!editTarget} onClose={() => setEditTarget(null)} initial={editTarget} />
      )}
      <ConfirmDialog
        open={!!deleteTarget}
