  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <div className={`p-1.5 rounded-none border border-(--arch-border) ${colorMap[data.type] || 'bg-blue-500/10 text-blue-500'} transition-colors duration-300 group-hover:border-(--arch-fg)`}>
          <Icon className="w-4 h-4" />
        </div>
        <div>
          <div className="text-xs font-bold text-(--arch-fg) font-mono tracking-wide uppercase">{data.label}</div>
          <div className="text-[10px] text-(--arch-muted) font-mono uppercase tracking-wider mt-0.5">ACTION</div>
        </div>
      </div>
