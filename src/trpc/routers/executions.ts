...
export const executionsRouter = createTRPCRouter({
  // List executions for the active team
  list: teamProcedure
    .input(
      z.object({
  ...
  // Get execution stats for the active team
  stats: teamProcedure
    .input(
      z.object({
        workflowId: z.string().optional(),
  ...
  retry: teamProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Check monthly execution limits against team plan
      const plan =
  ...
  timeline: teamProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      executions.forEach((exec) => {
        if (exec.startedAt) {
          const hourKey = exec.startedAt.toISOString().slice(0, 13);
