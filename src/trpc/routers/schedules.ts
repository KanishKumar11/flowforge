...
export const schedulesRouter = createTRPCRouter({
  ...
  presets: protectedProcedure.query(() => {
    return [
      ...
      { label: "Weekly (Monday)", expression: "0 9 * * 1", description: "Runs every Monday at 09:00" },
      { label: "Monthly (1st)", expression: "0 0 1 * *", description: "Runs on the 1st of each month" },
    ];
  }),
